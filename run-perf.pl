...
my sub get_test_cmd {
	...
	if (! $test_cmd) {
		...
		#
		my $tool_name = ( split(/_/, $test_name) )[0];
	...
	#
	$test_cmd = "$test_cmd -t $thread_count $cert_dir";
...
my sub do_test {
	...
	} elsif ( $test_name =~ /pkeyread/) {
		#
		# terse output for pkeyread command which
		# tests dhx key in pem format looks like
		# this:
		#	[dhx pem] 1.23456us
		# we need to extract 1.23456
		#
