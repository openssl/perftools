...
#define MIN_ELAPSED ossl_ms2time(100)
...
static ITEM *build_items(size_t n, const char *shared)
{
    ...
    if (n == 0 || n > SIZE_MAX / sizeof(*items))
        return NULL;

    items = OPENSSL_zalloc(n * sizeof(*items));;
    if (items == NULL)
        return NULL;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (shared != NULL) {
            ...
            snprintf(cn, sizeof(cn), "perf-%s-%zu",
                (i & 1) ? "crl" : "cert", i);
            name = cn;
        ...
        if (is_crl) {
            ...
            if (items[i].cert == NULL)
                goto err;
            if (X509_check_purpose(items[i].cert, -1, 0) <= 0)
                goto err;
        }
    }
